...
bool FRURDFParser::ProcessAttribute(const TCHAR* AttributeName, const TCHAR* AttributeValue)
{
	...
	// miyu: the condition used to be "if (!AttName.IsEmpty() && !AttValue.IsEmpty())"
   // I changed this to if (!AttName.IsEmpty() ... we had some `name=""` in our xml

   // miyu: when does this ever happen!?
   if (AttName.IsEmpty()) {
   ...
   // miyu: this code probably doesn't matter for us?
   ...
   // miyu: noop if within ignored tags
   if (Stack.Top().Equals("geometry_group")) {
      return true;
   }

	//Handles the sub categories for Links
   if (!Stack.Top().Equals("gazebo") &&
      !Stack.Top().Equals("transmission") &&
      !Stack.Top().Equals("material") &&
      !Stack.Top().Equals("link") &&
      Stack.FindLast("link") > Stack.FindLast("joint")) {
